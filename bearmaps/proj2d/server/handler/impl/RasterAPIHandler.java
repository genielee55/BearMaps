...
public class RasterAPIHandler extends APIRouteHandler<Map<String, Double>, Map<String, Object>> {
    ...
    public Map<String, Object> processRequest(Map<String, Double> requestParams, Response response) {
        ...
        double upLeftLon = requestParams.get("ullon");
        double downRightLon = requestParams.get("lrlon");
        double upLeftLat = requestParams.get("ullat");
        double downRightLat = requestParams.get("lrlat");
        double ROOT_LEFT = Constants.ROOT_ULLON;
        double ROOT_RIGHT = Constants.ROOT_LRLON;
        double ROOT_TOP = Constants.ROOT_ULLAT;
        double ROOT_BOT = Constants.ROOT_LRLAT;
        double width = requestParams.get("w");

        if (downRightLon > ROOT_LEFT && upLeftLon < ROOT_RIGHT && downRightLat < ROOT_TOP && upLeftLat > ROOT_BOT) {
            double xDist = downRightLon - upLeftLon;

            double lonDPP = xDist / width;
            int depth = getDepth(lonDPP);

            List<Double> xWidthLon = new ArrayList<>();
            List<Double> yHeightLat = new ArrayList<>();
            ...
            int xLeft = Math.max(0, (int) ((upLeftLon - ROOT_LEFT) / xWidthLon.get(depth)));
            int xRight = Math.min(((int) ((downRightLon - ROOT_LEFT) / xWidthLon.get(depth))),
                    ((int) (baseLon / xWidthLon.get(depth))) - 1);
            int yTop = Math.max(0, (int) ((ROOT_TOP - upLeftLat) / yHeightLat.get(depth)));
            int yBot = Math.min((int) ((ROOT_TOP - downRightLat) / yHeightLat.get(depth)),
                    (int) (baseLat / yHeightLat.get(depth)) - 1);
    ...
    private static int getDepth(double lonDPP) {
        ...
        for (int i = 0; i <= 7; i++) {
            ...
            double baseLonDPP = baseLon / units;
            if (baseLonDPP <= lonDPP || i == 7) {
                depth = i;
                break;
            }
        }
    ...
    public static void main(String[] args) {
        System.out.println(getDepth(0.00004291534423828125));
    }
